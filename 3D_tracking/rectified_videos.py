...
def process_video(video_path, calib_path, output_path):
    ...
    # CORRECT WAY: Use initUndistortRectifyMap
...
def main():
    video_files = glob.glob("raw_video/out*.mp4") # path to the video files
    output_dir = "3D_tracking/rectified_videos_result" # folder path where to save the rectified videos
    ...
    for video_path in video_files:
        ...
        if match:
            ...
            calib_path = os.path.join("3D_tracking", "camera_data", f"cam_{cam_index}", "calib", "camera_calib.json")
        else:
